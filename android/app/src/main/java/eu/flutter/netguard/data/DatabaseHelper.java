...
public class DatabaseHelper extends SQLiteOpenHelper {
        ...
        try {
            ...
            try {
                if (packet.getProtocol() == 6 /* TCP */ &&
                        packet.getDaddr() != null &&
                        packet.getDport() > 0 &&
                        packet.getUid() > 0 &&
                        "sni".equals(packet.getData())) {
                    int deleted = db.delete("log", "time > ?" +
                                    ...
                                    " AND uid = ?",
                            new String[]{
                                    Long.toString(packet.getTime() - SYN_SNI_DELAY),
                                    Long.toString(packet.getProtocol()),
                                    Long.toString(packet.getVersion()),
                                    "S", // SYN
                                    packet.getDaddr(),
                                    Long.toString(packet.getDport()),
                                    Long.toString(packet.getUid())
                            });
                    Log.i(TAG, "Deleted=" + deleted + " packet=" + packet + " dname=" + dname);
                }
                ContentValues cv = new ContentValues();
                cv.put("time", packet.getTime());
                cv.put("version", packet.getVersion());

                if (packet.getProtocol() < 0)
                    cv.putNull("protocol");
                else
                    cv.put("protocol", packet.getProtocol());

                cv.put("flags", packet.getFlags());

                cv.put("saddr", packet.getSaddr());
                if (packet.getSport() < 0)
                    cv.putNull("sport");
                else
                    cv.put("sport", packet.getSport());

                cv.put("daddr", packet.getDaddr());
                if (packet.getDport() < 0)
                    cv.putNull("dport");
                else
                    cv.put("dport", packet.getDport());
                ...
                cv.put("data", packet.getData());

                if (packet.getUid() < 0)
                    cv.putNull("uid");
                else
                    cv.put("uid", packet.getUid());

                cv.put("allowed", packet.getAllowed() ? 1 : 0);
            ...
            try {
                ...
                rows = db.update("access", cv, "uid = ? AND version = ? AND protocol = ? AND daddr = ? AND dport = ?",
                        new String[]{
                                ...
                                Long.toString(packet.getVersion()),
                                Long.toString(packet.getProtocol()),
                                dname == null ? packet.getDaddr() : dname,
                                Long.toString(packet.getDport())});
                ...
                if (rows == 0) {
                    ...
                    cv.put("version", packet.getVersion());
                    cv.put("protocol", packet.getProtocol());
                    cv.put("daddr", dname == null ? packet.getDaddr(): dname);
                    cv.put("dport", packet.getDport());
                    if (block < 0)
                        cv.put("block", block);
        ...
        try {
            ...
            try {
                ...
                String[] selectionArgs = new String[]{
                        Long.toString(usage.getUid()),
                        Long.toString(usage.getVersion()),
                        Long.toString(usage.getProtocol()),
                        dname == null ? usage.getDaddr() : dname,
                        Long.toString(usage.getDport())
                };
                ...
                try (Cursor cursor = db.query("access", new String[]{"sent", "received", "connections"}, selection, selectionArgs, null, null, null)) {
                    ...
                    ContentValues cv = new ContentValues();
                    cv.put("sent", sent + usage.getSent());
                    cv.put("received", received + usage.getReceived());
    ...
    public boolean insertDns(ResourceRecord rr) {
        ...
        try {
            ...
            try {
                ...
                ContentValues cv = new ContentValues();
                cv.put("time", rr.getTime());
                cv.put("ttl", ttl * 1000L);

                int rows = db.update("dns", cv, "qname = ? AND aname = ? AND resource = ?",
                        new String[]{rr.getQName(), rr.getAName(), rr.getResource()});

                if (rows == 0) {
                    cv.put("qname", rr.getQName());
                    cv.put("aname", rr.getAName());
                    cv.put("resource", rr.getResource());
                    cv.put("uid", rr.getUid());
    ...
    public String getQName(long uid, String ip) {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = this.getReadableDatabase();
